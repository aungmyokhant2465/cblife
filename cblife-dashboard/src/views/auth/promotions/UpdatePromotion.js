...
const UpdatePromotion = ({ homeAlert }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (promotion) {
      ...
      setTextValue(
        RichTextEditor.createValueFromString(promotion.paragraph, "html")
      );
      setPreview(promotion.image);
      if (promotion.image) {
        setOldImageName(
          promotion.image.substring(
            promotion.image.lastIndexOf("/") + 1,
            promotion.image.lenght
          )
        );
      }
    }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await promotionService.putPromotion(
        instanceToken.token,
        id,
        data
      );
      setpromotion(response.data);
      homeAlert("promotion Post have been updated.", false);
    } catch (error) {
      console.log(error);
    ...
    <>
      <div role="presentation" style={{ marginBlockEnd: "10px" }}>
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <Link to={`/promotions/${id}`}>Promotion (ID - {id})</Link>
          <span>Update</span>
        </Breadcrumbs>
      </div>
