...
const UpdateNews = ({ homeAlert }) => {
  ...
  useEffect(() => {
    if (news) {
      ...
      setTextValue(
        RichTextEditor.createValueFromString(news.paragraph, "html")
      );
      setPreview(news.image);
      if (news.image) {
        setOldImageName(
          news.image.substring(
            news.image.lastIndexOf("/") + 1,
            news.image.lenght
          )
        );
      }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (isImageChange) {
        ...
        data = { ...data, image: news.image };
      }

      const response = await newsService.putNews(instanceToken.token, id, data);
    ...
    <>
      <div role="presentation" style={{ marginBlockEnd: "10px" }}>
        <Breadcrumbs aria-label="breadcrumb">
          <Link to="/all_news">All News Post</Link>
